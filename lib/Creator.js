...
module.exports = class Creator extends EventEmitter {
  ...
  async create(cliOptions = {}, preset = null) {
    ...
    if (preset.key === 'blank-cli') {
      if (!name.endsWith('-cli')) {
        console.error(chalk.red(`The project name of this template needs to end with '-cli'`));
        process.exit(1);
      }
    }
    ...
    const rOptions = { projectName: name };
    if (preset.key === 'blank-cli') {
      rOptions.projectNameNocli = name.replace(/-cli$/, '');
    }
    const files = await generatorsAPI.render(tmpdir, rOptions);
